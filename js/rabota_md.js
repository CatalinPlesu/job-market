// --- Debug Script for rabota.md ---
console.log("Highlighting job links and pagination on rabota.md");

// Selector for job links based on config
const jobLinkSelector = 'a.vacancyShowPopup';
...
if (currentUrl.includes('page')) {
    ...
    const regexPattern = new RegExp(/page=\d+|\/\d+(?=[\/\s?#]|$)/, 'i'); // Use the escaped regex from Python
    ...
    firstPageUrl = 'https://www.rabota.md/ro/jobs-moldova/1';
...
window.removeRABOTA_MDHighlights = function() {
    jobLinks.forEach(removeHighlight);
    paginationLinks.forEach(removeHighlight);
    console.log("Highlights removed for rabota.md.");
};

console.log("Highlighting complete. Check the console for counts and the potential first page URL. Run removeRABOTA_MDHighlights() to remove highlights.");